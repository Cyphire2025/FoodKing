            ...
            <ul class="db-sidebar-nav-list" v-if="menus.length > 0" v-for="menu in menus" :key="menu">
                ...
                <li class="db-sidebar-nav-item" v-else @click.prevent="sidebarActive($event)">
                    <router-link :to="'/admin/' + menu.url" class="db-sidebar-nav-menu">
                        ...
                        <span class="text-base flex-auto">{{ menuLabel(menu) }}</span>
                    </router-link>
                </li>
                ...
                <li class="db-sidebar-nav-item" v-if="menu.children" v-for="children in menu.children"
                    ...
                    <router-link :to="'/admin/' + children.url" class="db-sidebar-nav-menu">
                        ...
                        <span class="text-base flex-auto">{{ menuLabel(children) }}</span>
                    </router-link>
                </li>
            </ul>
        </nav>
...
export default {
    ...
    computed: {
        ...
        menus: function () {
            const authMenus = this.$store.getters.authMenu || [];
            const visibleMenus = authMenus
                .map((menu) => {
                    const mappedMenu = { ...menu };
                    if (mappedMenu.children) {
                        mappedMenu.children = mappedMenu.children
                            .filter((child) => child.url !== 'offers')
                            .map((child) => {
                                if (child.url === 'coupons') {
                                    return { ...child, name: 'Offers', language: 'offers' };
                                }
                                return child;
                            });
                    }
                    if (mappedMenu.url === 'coupons') {
                        mappedMenu.name = 'Offers';
                        mappedMenu.language = 'offers';
                    }
                    return mappedMenu;
                })
                .filter((menu) => menu.url !== 'offers');
            return visibleMenus;
        },
        sidebar() {
    ...
    methods: {
        ...
        menuLabel(menu) {
            return menu?.name || this.$t('menu.' + menu.language);
        }
    }
}
</script>
