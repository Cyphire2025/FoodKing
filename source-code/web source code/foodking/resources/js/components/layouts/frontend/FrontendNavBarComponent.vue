<template>
    ...
    <header class="shadow-xs bg-white ff-header" :class="{ 'landing-header': isLandingPage, 'home-mobile-compact': isLandingPage }" ref="ffHeader">
        <div class="container flex flex-col lg:flex-row items-center justify-between">
            <div class="w-full flex justify-center lg:block lg:w-fit">
                <router-link :to="{ name: 'frontend.home' }">
                    <img class="w-32" :src="setting.theme_logo || '/images/theme/theme-logo.png'"
                        @error="$event.target.src = '/images/theme/theme-logo.png'" alt="logo">
                </router-link>
            </div>
            ...
            <div :class="isLandingPage ? 'hidden lg:flex' : 'flex'"
                class="flex-col items-center justify-end gap-3 w-full mt-4 lg:flex-row lg:w-fit lg:mt-0">
                <form v-if="!isLandingPage" @submit.prevent="search"
                    class="header-search-group group flex items-center justify-center border border-solid gap-2.5 px-4 w-full max-w-[340px] lg:max-w-none lg:w-52 h-10 lg:h-8 rounded-full transition border-[#EBDDE7] bg-white shadow-[0_10px_28px_rgba(109,51,92,0.10)] lg:shadow-none lg:border-[#EFF0F6] lg:bg-[#EFF0F6] focus-within:bg-white focus-within:border-primary focus-within:shadow-[0_12px_30px_rgba(109,51,92,0.14)] lg:focus-within:shadow-none">
                    <button type="submit" class="header-search-submit flex items-center justify-center text-primary/75">
                        <i class="lab lab-search-normal text-[20px] lg:text-base"></i>
                    </button>
                    <input type="search" v-model="searchItem" :placeholder="$t('button.search')"
                        class="header-search-field w-full h-full text-[15px] lg:text-xs appearance-none placeholder:font-normal placeholder:text-[#756781] text-heading">
                ...
                <div v-else class="dropdown-group">
                    ...
                    <div
                        ...
                        <div class="flex items-center gap-3 p-4 mb-2">
                            <figure
                                class="flex-shrink-0 relative z-10 w-[68px] h-[68px] rounded-full border-2 border-dashed border-white bg-gradient-to-t from-primary to-[#8B5A7E] before:absolute before:inset-0 before:-z-10 before:rounded-full before:scale-[1.03] before:bg-white">
                                <a
    ...
    <div id="order" v-if="orderNotificationStatus" ref="orderNotificationModal" class="modal active ff-modal">
        <div class="modal-dialog max-w-[360px] p-6 text-center relative">
            ...
            <router-link :to="{ path: '/admin/' + orderNotification.url }"
                class="db-btn h-[38px] shadow-[0px_6px_10px_rgba(109,_51,_92,_0.24)] bg-primary text-white">
