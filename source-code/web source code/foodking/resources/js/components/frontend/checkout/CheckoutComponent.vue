                    ...
                    <div class="rounded-2xl shadow-xs bg-white">
                                        ...
                                        <div class="w-full">
                                            ...
                                            <p v-if="Object.keys(cart.item_variations.variations).length !== 0"
                                                ...
                                                <span
                                                    ...
                                                    {{ localizeText(variationName) }}: {{ localizeText(variation) }}
                                                    <span
                        ...
                        <div class="p-4">
                            ...
                            <div class="mt-4 pt-4 border-t border-gray-100">
                                <h4 class="text-sm leading-6 font-semibold capitalize mb-3">
                                    {{ $t('label.select_payment_method') }}
                                </h4>
                                <div v-if="paymentMethods.length > 0" class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    <button v-for="method in paymentMethods" :key="method.slug" type="button"
                                        @click.prevent="selectPaymentMethod(method.slug)"
                                        class="w-full rounded-xl border py-3 px-4 text-left transition"
                                        :class="payment.selected === method.slug ? 'border-primary bg-primary/5' : 'border-gray-100 bg-white'">
                                        <span class="block text-sm font-semibold text-heading">{{ method.name }}</span>
                                        <span class="block text-xs text-paragraph">
                                            {{ method.slug === 'cash-on-delivery' ? 'Pay when your order arrives' : 'Pay online securely' }}
                                        </span>
                                    </button>
                                </div>
                                <p v-else class="text-sm text-danger">No payment method is enabled.</p>
                            </div>
                            <button type="button"
                                class="w-full rounded-3xl capitalize font-medium leading-6 py-3 text-white bg-primary"
                                :disabled="!payment.selected"
                                :class="!payment.selected ? 'opacity-60 cursor-not-allowed' : ''"
                                @click="orderSubmit">
...
export default {
    ...
    computed: {
        ...
        paymentMethods: function () {
            return this.setting.payment_gateways || [];
        },
    },
    mounted() {
    ...
    methods: {
        ...
        setDefaultPaymentMethod: function () {
            ...
            if (!this.payment.selected || !this.paymentMethods.some((method) => method.slug === this.payment.selected)) {
                this.selectPaymentMethod(this.paymentMethods[0].slug);
            }
        },
        selectPaymentMethod: function (method) {
            this.payment.selected = method;
            this.checkoutProps.form.checkout_payment_method = method;
        },
        calculateOrderTotal: function () {
            return parseFloat(this.subtotal + this.checkoutProps.form.delivery_charge - this.checkoutProps.form.discount).toFixed(this.setting.site_digit_after_decimal_point);
        },
        prepareOrderPayload: function () {
            this.checkoutProps.form.subtotal = this.subtotal;
            this.checkoutProps.form.total = this.calculateOrderTotal();
        ...
        orderSubmit: function () {
            ...
            this.$store.dispatch('frontendOrder/save', this.checkoutProps.form).then(orderResponse => {
                const paymentMethod = this.payment.selected;
                this.mapShow = false;
                ...
                this.checkoutProps.form.address_id = null;
                this.checkoutProps.form.coupon_id = null;
                this.checkoutProps.form.checkout_payment_method = null;
                this.checkoutProps.form.items = [];
