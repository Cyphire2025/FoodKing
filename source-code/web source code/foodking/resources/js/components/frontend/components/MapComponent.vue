<template>
    <div class="w-full max-w-[630px] rounded-2xl bg-white">
        ...
        <p v-if="mapError" class="text-xs leading-5 text-danger mb-4">{{ mapError }}</p>
    </div>
</template>
...
export default {
    ...
    methods: {
        resolveGoogleMapKey: async function () {
            if (ENV.GOOGLE_MAP_KEY) {
                return ENV.GOOGLE_MAP_KEY;
            }

            let setting = this.$store.getters['frontendSetting/lists'];
            if (!setting || Object.keys(setting).length === 0 || Array.isArray(setting)) {
                try {
                    const res = await this.$store.dispatch("frontendSetting/lists");
            ...
            return setting.site_google_map_key || "";
        },
        mainMap: async function (location) {
            try {
                this.mapError = "";
                const googleMapKey = await this.resolveGoogleMapKey();

                if (!googleMapKey) {
                    this.mapError = "Google Map key is missing.";
                    this.loading.isActive = false;
                    return;
                }

                const loader = new Loader(googleMapKey, options);
            ...
            if (this.setting.autocomplete) {
                ...
                this.mapError = "Unable to load Google Map. Please check the map key and browser location permission.";
                this.loading.isActive = false;
            }
        },
