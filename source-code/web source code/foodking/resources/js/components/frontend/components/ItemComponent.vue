        ...
        <div v-for="item in items" :key="item" v-show="type === null || type === item.item_type"
            ...
            <div class="product-card-list-content-group">
                <div class="product-card-list-header-group">
                    <h3 class="product-card-list-title">{{ localizeText(item.name) }}</h3>
                ...
                <p v-if="itemMetaText(item)" class="product-card-list-meta">
                    {{ itemMetaText(item) }}
                </p>
                <p class="product-card-list-describe">
                ...
                <div class="product-card-list-footer-group">
                    ...
                    <button @click.prevent="variationModalShow(item)" data-modal="#item-variation-modal"
                        class="product-card-list-cart-btn add-btn" :aria-label="$t('button.add')">
        ...
        <div v-for="item in items" :key="item" v-show="type === null || type === item.item_type"
            ...
            <div class="product-card-grid-content-group">
                <div class="product-card-grid-header-group">
                    <h3 class="product-card-grid-title">{{ localizeText(item.name) }}</h3>
                ...
                <p v-if="itemMetaText(item)" class="product-card-grid-meta">
                    {{ itemMetaText(item) }}
                </p>
                <p class="product-card-grid-describe">{{ localizeText(item.description) }}</p>
                <div class="product-card-grid-footer-group">
                    ...
                    <button @click.prevent="variationModalShow(item)" data-modal="#item-variation-modal"
                        class="product-card-grid-cart-btn add-btn" :aria-label="$t('button.add')">
                        <i class="fa-solid fa-plus"></i>
                    </button>
                </div>
            </div>
    ...
    <div id="item-info-modal" ref="itemInfoModal" class="modal ff-modal info-modal">
        <div class="modal-dialog" v-if="itemInfo">
            <div class="modal-header flex items-start gap-3">
                <h3 class="modal-title text-base font-medium">{{ localizeText(itemInfo.name) }}</h3>
                <button class="modal-close fa-regular fa-circle-xmark" @click.prevent="infoModalHide"></button>
            </div>
            <div class="modal-body">
                {{ localizeText(itemInfo.caution) }}
            </div>
        </div>
    </div>
                ...
                <div class="flex gap-4">
                    ...
                    <div class="flex-auto">
                        <div class="flex items-start gap-2 mb-1">
                            <h3 class="text-sm font-semibold capitalize">{{ localizeText(item.name) }}</h3>
                            <button v-if="item.caution" type="button" class="info-btn mt-0.5 flex items-start"
                                data-modal="#item-info-modal" @click.prevent="infoModalShow(item.name, item.caution)">
                                <i class="lab lab-information font-fill-paragraph transition lab-font-size-16"></i>
                            </button>
                        </div>
                        <p v-if="itemMetaText(item)" class="text-[11px] font-semibold text-[#8A6A6A] mb-2">
                            {{ itemMetaText(item) }}
                        </p>
                        <p class="text-xs mb-2">{{ localizeText(item.description) }}</p>
                        <h4 class="text-sm font-semibold font-roboto">
                    ...
                    <div class="row">
                        <div v-for="itemAttribute in item.itemAttributes" class="col-12 sm:col-6">
                            <label class="text-sm leading-6 block font-medium capitalize mb-1.5 text-heading">
                                {{ localizeText(itemAttribute.name) }}
                            </label>
                            <div class="relative">
                                <i
                                ...
                                <select
                                    ...
                                    <option :value="variation.id" v-for="variation in item.variations[itemAttribute.id]"
                                        :key="variation">{{ localizeText(variation.name) }} +{{ variation.currency_price }}
                                    </option>
                                </select>
                            </div>
                        </div>
                                    ...
                                    <label
                                        ...
                                        <div class="custom-radio sm">
                                            <input :value="variation.id"
                                                @click="changeVariation(variation.item_attribute_id, variation.id, localizeText(variation.name), variation.convert_price)"
                                        ...
                                        <div class="ltr:text-left rtl:text-right">
                                            <h3 class="block capitalize text-xs text-heading">
                                                {{ textShortener(localizeText(variation.name), 15) }}</h3>
                                    ...
                                    <label :for="extra.id + extra.name"
                                        ...
                                        <div class="custom-checkbox w-3 h-3">
                                            <input :id="extra.id + extra.name"
                                                @change.prevent="changeExtra($event, extra.id, localizeText(extra.name))"
                                        ...
                                        <div>
                                            <h3 class="block capitalize mb-1 text-xs text-heading">
                                                {{ textShortener(localizeText(extra.name), 15) }}</h3>
                                    ...
                                    <div @click.prevent="changeAddon(addon)"
                                        ...
                                        <div class="rounded-r-lg w-full py-1 px-2">
                                            <span
                                                ...
                                                {{ localizeText(addon.addon_item_name) }}
                                            </span>
                                            <p v-if="addon.variation_names.length > 0"
                                                ...
                                                <span v-for="(variation, index) in addon.variation_names">
                                                    {{ textShortener(localizeText(variation.name), 8) }}
...
import 'swiper/css';
import { translateFrontendObject, translateFrontendText } from "../../../services/frontendLocalizer";
...
export default {
    ...
    props: {
        ...
        gridClass: {
            type: String,
            default: "grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 lg:gap-6",
        },
    },
    data() {
        return {
    ...
    methods: {
        ...
        itemMetaText: function (item) {
            const parts = [];
            const size = `${item.size || ""}`.trim().replace(/\s*-+\s*$/, "");
        ...
        variationModalShow: function (selectedItem) {
            ...
            this.$store.dispatch('frontendItem/details', selectedItem.id)
                .then((res) => {
                    ...
                    const item = translateFrontendObject(res.data.data);
                    this.item = item;
                    ...
                    if (this.item.itemAttributes.length > 0) {
                        _.forEach(this.item.itemAttributes, (element) => {
                            if (typeof this.item.variations[element.id][0] !== "undefined") {
                                ...
                                this.temp.item_variations.names[this.localizeText(element.name)] = this.localizeText(this.item.variations[element.id][0].name);
                    ...
                    this.temp.name = this.localizeText(this.item.name);
        ...
        changeVariation: function (attributeId, variationId, variationName, variationPrice) {
            ...
            _.forEach(this.item.itemAttributes, (element) => {
                if (element.id === attributeId) {
                    this.temp.item_variations.names[this.localizeText(element.name)] = variationName;
                }
            });
        ...
        changeVariationAdjust: function (attributeId, variationId) {
            _.forEach(this.item.variations[attributeId], (variation) => {
                if (variation.id === variationId) {
                    this.changeVariation(attributeId, variationId, this.localizeText(variation.name), variation.convert_price);
                }
            });
        },
        ...
        changeAddon: function (addon) {
            if (typeof this.addons[addon.id] === "undefined") {
                this.addons[addon.id] = {
                    name: this.localizeText(addon.addon_item_name),
                    image: addon.thumb,
                ...
                if (addon.variation_names.length > 0) {
                    _.forEach(addon.variation_names, (variation) => {
                        this.addons[addon.id].item_variations.names[this.localizeText(variation.attribute_name)] = this.localizeText(variation.name);
                    });
                }
            } else {
