        ...
        <div class="container max-w-[360px] py-6 p-4 mb-6 sm:px-6 shadow-xs rounded-2xl bg-white">
            ...
            <form @submit.prevent="login">
                ...
                <div v-if="socialLoginProviders.length > 0" class="flex justify-center flex-wrap gap-[10px] mb-6">
                    <div v-for="socialLoginProvider in socialLoginProviders" :key="socialLoginProvider.id"
                        @click.prevent="socialLogin(socialLoginProvider.slug)"
                        class="flex items-center justify-center gap-1.5 bg-[#F7F7FC] px-3 h-10 rounded-lg cursor-pointer">
                        <img class="h-6 w-6 rounded-full" :src="socialLoginProvider.image"
                            :alt="socialLoginProvider.name" />
