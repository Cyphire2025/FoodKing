...
class ItemService
{
    ...
    public function store(ItemRequest $request)
    {
        try {
            DB::transaction(function () use ($request) {
                $this->item = Item::create($this->itemPayload($request));
                if ($request->image) {
    ...
    {
        try {
            $selectedItem = $this->todaySpecialService->getSelectedItem();

            if ($selectedItem) {
                return collect([$selectedItem]);
            }
    ...
    private function itemPayload(ItemRequest $request): array
    {
        $payload = $request->validated();
        $payload['slug'] = Str::slug($request->name);
        $payload['size'] = $request->filled('size') ? $request->input('size') : null;
        $payload['calories'] = $request->filled('calories') ? (int)$request->input('calories') : null;
