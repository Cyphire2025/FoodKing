...
class CouponService
{
    ...
    public function couponDateWise(): \Illuminate\Database\Eloquent\Collection
    {
        try {
            ...
            return Coupon::all()->filter(function ($item) use ($userId) {
                if (!AppLibrary::isBetweenDate($item->start_date, $item->end_date)) {
                    return false;
                }

                // If user is logged in and coupon has a per-user limit, check usage
                if ($userId && $item->limit_per_user > 0) {
                    $usedCount = OrderCoupon::where(['user_id' => $userId, 'coupon_id' => $item->id])->count();
                    if ($usedCount >= $item->limit_per_user) {
                        return false;
                    }
                }
            ...
            if ($coupon) {
                if ($coupon->minimum_order > $request->total) {
                    ...
                    if (strtotime($coupon->end_date) >= strtotime(Carbon::now())) {
                        ...
                        if ($coupon->limit_per_user > 0 && $coupon->limit_per_user <= $ordered_coupon_count) {
