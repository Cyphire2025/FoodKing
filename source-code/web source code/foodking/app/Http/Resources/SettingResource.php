...
class SettingResource extends JsonResource
{
    ...
    {
        $appDesign = app(AppDesignService::class)->list();
        ...
        return [
            ...
            'payment_gateways'                     => \App\Models\PaymentGateway::where('status', \App\Enums\Activity::ENABLE)
                ->where('slug', '!=', 'credit')
                ->when(!config('kahumbo.cod_enabled'), fn($query) => $query->where('slug', '!=', 'cash-on-delivery'))
                ->get(['id', 'name', 'slug'])
                ->map(fn($gateway) => [
                    'id'    => $gateway->id,
                    'name'  => $gateway->name,
            ...
            'app_intro_media_url'                  => $this->appIntroMedia()?->media_url,
            'app_intro_media_type'                 => $this->appIntroMedia()?->media_type,
            'latest_update'                        => app(AppUpdateService::class)->first(),
            'latest_updates'                       => app(AppUpdateService::class)->list(),
            'app_primary_color'                    => $appDesign['primary_color'],
            'app_banner_1_url'                     => $appDesign['banner_1_url'],
            'app_banner_2_url'                     => $appDesign['banner_2_url'],
    ...
    public function appIntroMedia()
    {
        return AppIntroSetting::where(['group' => 'app_intro', 'key' => 'app_intro_media'])->first();
    }
}
